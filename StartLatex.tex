...
\begin{tikzpicture}
    ...
    \begin{axis}[xmin= -2, xmax = 2, ymin=-5, ymax= 5,%color
        axis lines = middle,
        xlabel = $x$,
        ylabel = $y$,
        title = {Graph of $y = x^{2}$ and $y = x^{3}$}]
    \addplot[color=pink, dashed, mark=*, samples= 50]{(x^2)};
    \addplot[color=blue, dotted, mark=*, samples= 50]{(x^3)};
    \end{axis}
\end{tikzpicture}
